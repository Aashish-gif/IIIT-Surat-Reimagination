...
import { Star, GitFork, Flame, TrendingUp } from 'lucide-react'
...
interface RepoMomentumCardProps {
    isLoading?: boolean
    data?: Repository[]
...
const getMomentumColor = (momentum: number) => {
    if (momentum >= 80) return 'from-orange-500 to-red-500'
    if (momentum >= 60) return 'from-yellow-500 to-orange-500'
    if (momentum >= 40) return 'from-emerald-500 to-cyan-500'
    return 'from-cyan-500 to-blue-500'
}

export function RepoMomentumCard({ isLoading, data, highlightedSkill }: RepoMomentumCardProps) {
    const repos = data || []

    return (
        <motion.div className="glassmorphic col-span-3 row-span-1 p-6 neon-glow">
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {repos.map((repo, idx) => {
                        ...
                        const momentum = repo.momentum || Math.floor((repo.recentCommits || 0) * 10)
                        ...
                        return (
                            ...
                            >
                                {/* Trending Badge */}
                                {isTrending && (
                                    <div className="absolute top-3 right-3 flex items-center gap-1 px-2 py-0.5 rounded-full bg-orange-500/10 border border-orange-500/20 text-[10px] text-orange-400 font-bold uppercase tracking-tighter">
                                        <motion.div
                                ...
                                <div className="mb-4">
                                    <div className="flex items-center justify-between mb-1.5">
                                        ...
                                        <span className="text-[10px] font-mono text-primary">{momentum}%</span>
                                    </div>
                                    <div className="w-full h-2 bg-muted/30 rounded-full overflow-hidden">
                                        <motion.div
                                            ...
                                            className={`h-full bg-gradient-to-r ${getMomentumColor(momentum)}`}
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                                    ...
                                    <span>{repo.recentCommits || 0} commits in last 48h</span>
                                </div>
                            </motion.div>
                        )
                    })}
